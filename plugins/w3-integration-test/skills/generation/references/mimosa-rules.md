...
## §s3-abnormal-only — S3 (バリデーション) = 異常 cases + the WHOLE BVA boundary cluster

🔴 `3. バリデーション確認` (S3) holds the inputs the screen must REJECT (**異常**) **plus the full BVA
boundary cluster kept together** — the N-1 / N / N+1 triple stays in S3 even though N-1 / N are 正常
(see §BVA). Any OTHER **正常 (valid / accepted / business-success) case that is NOT a boundary value
moves to `4.1 業務処理の確認` (S4.1)**. Consequences: a boundary triple (e.g. 499 / 500 / 501) stays
TOGETHER in S3; a required-empty / over-limit / wrong-type case is 異常 → S3; a plain valid submit is
正常 → S4.1. `build_csv.py`'s `verify()` warns on a 正常 TC under the `3.` marker **unless** it is a
boundary-value case (中分類/小分類 = 境界値 / 上限ちょうど / 上限未満 / 下限).

## §BVA — boundary value analysis (the WHOLE boundary cluster goes in S3)

🔴 For any action/filter with a MAX, cover the boundary at N-1 / N / N+1 and keep **all three
together in S3 (バリデーション確認)** — do NOT split the cluster off to S4.1. (Overrides the generic
"正常 → S4.1": a boundary value is validation testing, so the whole triple lives in the validation
section.) 中分類 = the action / `境界値`, 小分類 = the case:
- **`上限未満（(N-1)…）`** → succeeds (**正常**, in S3).
- **`上限ちょうど（N…）`** → succeeds at the limit (**正常**, in S3).
- **`上限超過（(N+1)…）`** → rejected → E024/E028 (**異常**, in S3).

Constants: `MAX_SELECTION=200`→E024, `MAX_TRANSIT_SELECTION=500`→E028. **Confirm the operator** in
code: `MAX < n` ⇒ n=MAX PASS (正常), n=MAX+1 FAIL (異常). All three rows sit in S3 (e.g. 499 / 500 / 501).

**Date-range filter** (list screen with `適用` + from/to date, `reload()` checks a range max, e.g.
`HISTORY_SEARCH_MAX_RANGE=90` → E027): same — the **whole cluster in S3**:
- **正常 (S3):** `上限未満（(MAX-1)日）` + `上限ちょうど（MAX日）` happy reloads (中=`適用`).
- **異常 (S3):** `上限超過（(MAX+1)日）` → E027, `開始日 > 終了日`（dateDiff<0 → E027）, and the
...
**Assign by status** (§s3-abnormal-only): 異常 (rejection) → **S3**; a **boundary-PASS (境界値:
上限ちょうど / 下限ちょうど) stays in S3** next to its 異常 counterpart (§BVA); only a non-boundary
accepted case (valid submit / only-required) → **S4.1**. 中分類 = field name
(`商品コード`/`入荷限界設定日数`…), 小分類 = the case.

🔴 **Order within S3 (least-confusing for the tester):** put **single-field** validation first (one
field at a time), then **cross-field / whole-section / 全必須** validation below (e.g. 全必須項目未入力,
a fee-simulation multi-field guard). Simple cases on top, compound/related-item cases at the bottom.
...
- `全項目入力（必須＋任意すべて）` — fill EVERY field (required + optional) → 登録/更新 succeeds and all
  entered values **including the optional ones** persist (正常 → S4.1; pair with §verify-db so the DB
  check covers the optional columns too).

**Per string field with `maxlength = N`** (N = that field's real limit, e.g. 100 / 500 — read `$rules`):
- `上限超過（N+1文字）` → length error (異常 → S3) · `境界値 ― 上限ちょうど（N文字）` → accepted (正常 → **S3**, kept with the over-limit case).

**Per numeric field** (use the field's real bounds):
- `負数（マイナス値）` — if negative not allowed → error (異常 → S3).
- `ゼロ（0）` — if 0 not allowed → error (異常 → S3); if allowed it is the `下限ちょうど` boundary (正常 → **S3**).
- `下限未満` / `上限超過（桁あふれ）` (e.g. int max 2147483647) → error (異常 → S3).
- `数値以外 ― 文字（文字列入力）` / `数値以外 ― 記号（特殊文字）` / `数値以外 ― 小数（整数項目）` /
  `数値以外 ― 指数表記（"e" / "1e3"）` → 「<field>は半角数値で入力してください。」 / UI block (異常 → S3).
  (The HTML number input silently accepts `e` / `+` / `-` — test it explicitly.)
- `境界値 ― 下限ちょうど` / `上限ちょうど（N）` → accepted (正常 → **S3**, kept with the out-of-range 異常).

**Per date / datetime field (if any):**
- `不正な型（文字・記号入力）` — type letters / special chars into the date field → format error
  (異常 → S3). `不正な形式（YYYY-MM-DD でない）` — a malformed date string → format error (異常 → S3).
  Quote the real message, e.g. **「有効な日付を年月日（YYYY-MM-DD）形式で入力してください。」（E040）**.
- A valid in-range date rides the success TC (正常 → S4.1). (A range/boundary on dates → §BVA, in S3.)
...
🚫 Do NOT collapse to "required only" — the #1 miss on form screens; a form's S3 (異常 + each field's
boundary-PASS) is substantial. Generate only cases the field's type/constraints actually support (no email TC if no
...
🔴 **Form/edit screen override:** the field-input sub-flows (`「マスタから選択」` master-select +
its 「キャンセル」, `「＋商品を追加」` add-row / row-delete, accordion 全て開く/全て閉じる) come **before**
the 登録/更新 success group — the tester learns to fill the form, THEN submits. So a form's order is:
初期表示 → field sub-flows → **登録/更新 success (+ DB verify §verify-db + concurrent §concurrent-conflict)**
→ remaining groups. (On a list screen, keep the default order below: mutation buttons early.)
...
2. **Real mutation buttons** (full per-button: 読み込み中 → 成功(I001) → APIエラー → 確認モーダル if Q-code).
   This group also holds the **non-boundary 正常 cases** (§s3-abnormal-only): valid-input success for
   the create/update action (the BVA boundary cluster itself — N-1 / N / N+1, boundary-maxlength —
   stays in S3). **On a form/edit/create screen the success TC also adds a DB-persistence verify
   step** (§verify-db).
