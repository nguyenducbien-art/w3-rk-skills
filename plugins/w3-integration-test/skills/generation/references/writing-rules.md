...
- 🔴 **One outcome per line — never a long compound EXPECTED.** When a single step produces several
  outcomes (the standard mutation-error result is the worst offender: *error modal + loading hides +
  nothing saved + stays on the form*), do NOT cram them into one 3–4-clause sentence. Split into
  sub-numbered lines, one short sentence each:
  ```
  3.1 The error modal 「<message>」 is displayed.
  3.2 The loading indicator is hidden.
  3.3 Nothing is saved; the screen stays on the form.
  ```
  (Each sub-line keeps the top-level step number — STEP 3 → EXPECTED 3.1/3.2/3.3. `verify()` warns on
  a bundled-outcome line.)
- 🔴 **Validation errors quote the message VERBATIM.** An S3 (バリデーション) error TC's EXPECTED must
  quote the real message 「…」 from the BE (`MessageController` / `$rules` / the `<Entity>Observer`,
  §be-first) — never a paraphrase like *"a validation message indicating that X is required"*. Put the
  exact 「…」 (+ the code E0xx if shown). The only S3 case without a 「…」 is a pure UI block (e.g. a
  number input refusing a letter — no modal). `verify()` warns on an S3 error EXPECTED that mentions
  an error/message but quotes no 「…」.
...
- **検索/選択 (master-select) modal cancel — ONE representative TC.** A screen with `「マスタから選択」`
  buttons opens selection modals; add **one** representative TC (中分類 = the action): open a select
  modal → click 「キャンセル」 → the modal closes, no selection is applied, the field stays unchanged.
  One is enough for the whole screen — do NOT repeat it for every select modal, and the `×` close icon
  stays excluded (`test-categories.md` §7).
