...
compound precondition — *open/land on a screen → (don't) select rows → click a button → reach the
...
✅ One action per step (apply to BOTH the EN and JP columns):
...
- The resulting screen/mode ("opens 434 in new-registration mode / empty form") is an OUTCOME —
  keep it OUT of the click action. The landing screen is verified by the test's STEPS/EXPECTED,
  not narrated inside a PRE line.
- This is the navigation-from-upstream shape (a form opened from a parent list; cf. §ids): the PRE
  lists the navigation actions atomically, it never bundles "click X to open Y in mode Z".
- `build_csv.py`'s `verify()` warns when a single PRE line contains both a `(screen_id=…)` reference
  and a click — the tell-tale of a crammed navigation precondition; split it.
