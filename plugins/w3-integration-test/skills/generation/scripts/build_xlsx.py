...
def set_side(cell, *, top=None, bottom=None, left=None, right=None):
    """Override only the named border side(s); keep the others (and diagonal/outline) as-is."""
    o = cell.border
    cell.border = Border(
        top=top if top is not None else o.top,
        bottom=bottom if bottom is not None else o.bottom,
        left=left if left is not None else o.left,
        right=right if right is not None else o.right,
        diagonal=o.diagonal, outline=o.outline)


def set_right_edge(ws, row, col, side):
    """Draw a bold RIGHT edge at (row, col). When that cell is the right end of a horizontal merge
    (the result columns are merged AS:AW), openpyxl drops a border set on the non-anchor cell — so
    also paint the LEFT border of the next column, which renders the very same vertical line and is
    immune to the merge (col+1 is outside every result merge)."""
    set_side(ws.cell(row, col), right=side)
    set_side(ws.cell(row, col + 1), left=side)


# Static header band on 3.テストケース (above DATA_START — not regenerated by the data loop):
HEADER_TOP_ROW = 16       # gray column header (№ 大分類 … 確認事項) starts here (merged A16:K17)
HEADER_BOT_ROW = 17       # …and ends here
